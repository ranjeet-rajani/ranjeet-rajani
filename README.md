![Ranjeet Kumar Rajani — Business Analyst, ERP & SAP S/4HANA, Commercial Analytics](banner.png)

<p align="center">
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black" alt="Power BI">
  <img src="https://img.shields.io/badge/DAX-2E5EAA?style=flat-square" alt="DAX">
  <img src="https://img.shields.io/badge/SQL-CC2927?style=flat-square&logo=microsoftsqlserver&logoColor=white" alt="SQL">
  <img src="https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white" alt="R">
  <img src="https://img.shields.io/badge/SAP%20S%2F4HANA-0FAAFF?style=flat-square&logo=sap&logoColor=white" alt="SAP S/4HANA">
  <img src="https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white" alt="Tableau">
</p>

## About

Business Analyst with hands-on **SAP S/4HANA (FI/CO, MM, PP)** configuration experience and an **MBA in Enterprise Resource Planning**, translating 20+ years of pharmaceutical commercial leadership into enterprise systems and analytics.

I spent two decades running sales teams, P&Ls, and product launches — which means I know which questions are worth asking before I open a dataset. The analytics is what lets me answer them.

Based in Fairfield, Iowa · Authorized to work in the US

---

## Projects

### 🏥 [Pharma Commercial Analytics Command Center](https://github.com/ranjeet-rajani/pharma-commercial-analytics-command-center)

`Power BI` · `DAX` · `SQL` · `Power Query` · `Star Schema Modeling`

A seven-page executive report that traces a **$4.34M revenue shortfall** to its operational root causes and assigns each one an owner.

- 9-table star schema, 16 single-direction relationships, **45 DAX measures** reconciled to source
- Found that **30% of the gap ($1.31M) traced to inventory stockouts**, not sales execution
- Classified all 20 territories by root cause — supply, competitive, or execution — each mapped to an accountable function

> The interesting finding wasn't *who* missed quota. Every territory did. It was *why* — because seven of them didn't have product on the shelf, and coaching those teams would have achieved nothing.

### 🚲 [Cyclistic Bike-Share Analysis](https://github.com/ranjeet-rajani/cyclistic-bike-share-analysis)

`R` · `tidyverse` · `ggplot2` · `Google Data Analytics Capstone`

Analysis of **4.3M bike-share rides** comparing casual riders and annual members to support a membership conversion strategy.

- Members ride **1.8× more often**; casual riders ride **1.9× longer**
- Casual ridership swings **8× seasonally** against under 3× for members
- Five visualizations and three targeted marketing recommendations

---

## Skills

| | |
|---|---|
| **ERP / SAP** | SAP S/4HANA · FI/CO · MM · PP · Procure-to-Pay configuration · Business process mapping · Gap analysis |
| **Analytics** | Power BI (DAX, Power Query, star schema modeling) · SQL · R / tidyverse · Tableau · Advanced Excel |
| **Business** | Requirements gathering · Stakeholder reporting · KPI definition · Sales forecasting · P&L management |

---

## Background

**MBA, Enterprise Resource Planning (ERP) & SAP** — Maharishi International University, Fairfield IA · *expected September 2026*

**Google Data Analytics Professional Certificate** — SQL, R, Tableau, data cleaning, visualization

**20+ years in pharmaceutical commercial roles** — Ferozsons Laboratories, CCL Pharmaceuticals, Getz Pharma. Regional and zonal sales leadership, national product launches, P&L ownership across Pakistan.

---

## Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ranjeet-rajani/)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:ranjeetkumarrajanii@gmail.com)
[![Kaggle](https://img.shields.io/badge/Kaggle-20BEFF?style=flat-square&logo=kaggle&logoColor=white)](https://www.kaggle.com/ranjeetkumarrajani)

Open to Business Analyst, Commercial Analyst, Sales Operations Analyst, Healthcare Analyst, and SAP/ERP Business Analyst roles.
